---
title: My-Second-Blog
date: 2026-04-19
author: Jingru Qu
summary: Discussed and evaluated ideas, selected the anonymous emotional support community concept, defined target users, and planned development using a ticket-based task breakdown.
---
### Final Idea Selection  

After exploring multiple ideas in Week 6, our group selected the anonymous emotional support community as the final concept after discussion and comparison. The decision was based on three key factors: clarity of user needs, uniqueness of interaction, and feasibility for development.   

This concept targets users who prefer low-pressure and anonymous interaction, especially those who feel uncomfortable revealing their identity online. These users may feel socially fatigued, avoid traditional social media, or simply want casual and temporary conversations.   

Unlike traditional social platforms that focus on identity and long-term relationships, this design emphasizes temporary interaction and lightweight connections. It explores how users can still experience a sense of connection without stable identities.  

---
### Target Users    

The target users are people who prefer low-pressure and anonymous social interaction, rather than being defined by a specific age group.  

These users typically include individuals who:  
- Do not enjoy traditional social media environments  
- Do not want to reveal their real identity  
- Prefer casual and effortless conversations  
- Feel occasional loneliness but avoid social pressure  

Their core needs are:  
- Anonymity  
- Low-pressure interaction  
- Short-term and flexible communication  
- A space to express thoughts freely (emotional outlet)    

This concept focuses more on behavioral patterns and emotional needs rather than demographic categories, allowing the platform to be flexible and inclusive.  

---
### Group Discussion  

During the group discussion, we evaluated each idea based on uniqueness, user value, and scope of implementation. The anonymous community stood out because it offers a different model of interaction compared to typical social platforms.  

The core interaction is based on randomness and anonymity. Users are assigned a random identity and enter rooms without knowing others’ identities. This reduces social pressure and allows more spontaneous communication.    

A key feature is the repeated encounter system, which informs users if they have met before. This introduces a subtle sense of continuity without requiring fixed relationships, creating a balance between unfamiliarity and familiarity.  

---
### Project Planning (Tickets)   

To structure the development process, we divided the concept into smaller tasks using a ticket-based approach. This allows clearer planning and more manageable implementation.  

The core tasks include:   
- Random room matching  
- Room creation and joining  
- Chat interaction system  
- Repeated encounter tracking  
- Moderation mechanisms and basic system control features(ensure a safe environment. )

---
### Conclusion

This week helped establish a clear and focused design direction through group collaboration. The selected concept not only responds to user needs but also offers a distinct interaction model.  

The use of tickets also clarified how the system can be developed step by step, making the project more feasible.  The next step is to translate this concept into a working prototype and test how the interaction functions in practice.  
